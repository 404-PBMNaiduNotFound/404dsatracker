...
export const Route = createFileRoute("/reset-password")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Choose a new password for your DSA⁴⁰⁴ account." },
      { property: "og:title", content: "Reset password — DSA⁴⁰⁴" },
      { property: "og:description", content: "Choose a new password for your tracker account." },
    ],
  }),
  // Firebase's password reset link lands here with ?mode=resetPassword&oobCode=...
  validateSearch: (s: Record<string, unknown>) => ({
    oobCode: (s.oobCode as string) || "",
  }),
...
function ResetPassword() {
  ...
  async function submit() {
    ...
    try {
      ...
      void navigate({ to: "/auth", search: { next: "/today" } });
    } catch (e) {
